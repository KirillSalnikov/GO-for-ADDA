...
| `--fp` | Использовать аналитическую модель Фабри-Перо вместо GO-отражений |
...
| GO-отражения + Кирхгоф | **0.353** | **0.352** |
| `--fp` (Фабри-Перо) | 0.357 | 0.356 |

GO-отражения с дифракционным сглаживанием Кирхгофа **лучше** FP при β=0° на большой частице (0.353 vs 0.357).
...
| GO-отражения + Кирхгоф | 0.307 | 0.267 |
| `--fp` (0 пар, = norefl) | 0.282 | 0.252 |

При β=30° GO-отражения по-прежнему ухудшают результат (+9% vs norefl). Проблема в амплитуде/поляризации из матрицы Джонса GO, а не в границах пучков (Кирхгоф-сглаживание не решает проблему).
...
- `AccumulateReflectedBeams(segments, incidentDir, maxJonesNorm)` — обрабатывает GO-сегменты с nActs=1 (однократно отражённые пучки). Аддитивное накопление (+=) поверх прямой волны. Per-dipole фаза: обратная проекция на грань отражения → входная грань → OP = FAR_ZONE + incDir·P + n·(d₁+d₂). Фильтр по норме Джонса (|J| > 0.5) отсеивает пучки полного внутреннего отражения. **Дифракционное сглаживание**: гибридный подход — для широких пучков (число Френеля N_F > 2.5) используется произведение полуплоскостных весов Френеля, для узких пучков — точный интеграл Кирхгофа (теорема Грина: 2D интеграл Френеля → 1D контурные интегралы по рёбрам полигона, 8-точечная квадратура Гаусса-Лежандра). Координаты — в плоскости, перпендикулярной пучку (e_perp, e_par), обход полигона CCW
...
### GO-отражения (nActs=1): ухудшение при наклонном падении

Метод `AccumulateReflectedBeams` добавляет отражённую волну из GO-трейсинга (nActs=1 сегменты) поверх прямой волны, вычисленной per-facet PW.

**Попытка исправления фаз (per-dipole phase)**: для каждого диполя, покрытого отражённым пучком, фаза вычисляется аналитически:
1. Обратная проекция диполя вдоль -d_refl на грань отражения → точка R
2. Восстановление d_inc по формуле отражения: d_inc = d_refl - 2·(d_refl·n_B)·n_B
3. Трассировка от R вдоль -d_inc до входной грани → точка P
4. OP = FAR_ZONE + incDir·P + n·(|P→R| + |R→dip|)

Результат: per-dipole фаза **не улучшает** RE₀ — значения совпадают со старым методом (beam-center) для выпуклых частиц и даже ухудшаются для вогнутых (обратная трассировка попадает на внутренние вогнутые грани вместо входной грани).

**Вывод**: проблема не в фазовой опорной точке. GO-отражения при наклонном падении принципиально деградируют поле, потому что:
- Амплитуда/поляризация из матрицы Джонса GO несогласована с аналитическими коэффициентами Френеля в per-facet PW
- GO-пучки имеют резкие границы (ступенчатый профиль), создавая разрывы в поле
- Для вогнутых частиц внутренние отражения от вогнутых граней добавляют шум

**Рекомендации:**
- При β = 0°: GO-отражения с Кирхгофом дают лучший результат (0.353 vs 0.357 FP на 20×10)
- При β > 0°: использовать `--norefl` (GO-отражения ухудшают начальное поле)
- По умолчанию (без флагов) используются GO-отражения с Кирхгофом, `--fp` для аналитической модели Фабри-Перо, `--norefl` для отключения всех отражений
...
### Дифракционное сглаживание GO-отражений (Кирхгоф)

GO-пучки имеют резкие полигональные границы. Для корректного описания дифракции на краях апертуры реализовано:

1. **Произведение полуплоскостей Френеля** — для широких пучков (maxFresnel > 2.5): для каждого ребра полигона вычисляется вес Френеля W(d, z, λ_eff), итоговый вес = произведение по всем рёбрам. Хорошо работает когда рёбра далеко от точки наблюдения (N_F >> 1).

2. **Интеграл Кирхгофа** — для узких пучков (maxFresnel ≤ 2.5): 2D интеграл Френеля по полигональной апертуре через теорему Грина сводится к контурным интегралам: U/U₀ = (-i/2) ∮ Q(X,Y) dY, где Q = exp(iπY²/2)·[(1+i)/2 + C(X) + iS(X)]. 8-точечная квадратура Гаусса-Лежандра по каждому ребру.
...
Результат: помогает при β=0° (0.353 vs 0.384 norefl), но не решает проблему при наклонном падении (β=30°: 0.307 vs 0.282 norefl).
