...
| `--rmax R` | Макс. \|Rs\|,\|Rp\| для аналитических отражений (по умолчанию 0.25) |
| `--noinit` | Вывести только shape-файл, без field-файлов (для старта с x₀=0) |
...
### Влияние начального поля на число итераций DDA

Начальная невязка RE₀ показывает, насколько начальное приближение близко к решению. Но ключевой вопрос — сколько **итераций** экономится при лучшем начальном поле?

**Важно**: аргумент `-eps` в ADDA — это **показатель степени**, не значение. `-eps 5` означает ε = 10⁻⁵. Ошибочное использование `-eps 1e-5` даёт порог ≈ 1, и ADDA не итерирует вообще.

#### QMR (дефолтный солвер ADDA) — начальное поле не помогает

| Частица | m | dpl | Диполи | Init | RE₀ | Итерации |
|---------|-----|-----|--------|------|-----|----------|
| Куб 5µm | 1.3116 | 10 | 831K | analytical | 0.327 | 1338 |
| Куб 5µm | 1.3116 | 10 | 831K | norefl | 0.362 | 1541 |
| Куб 5µm | 1.3116 | 10 | 831K | **default (x₀=0)** | 1.000 | **1305** |
| Куб 3µm | 1.50 | 10 | 178K | analytical | 0.458 | 4283 |
| Куб 3µm | 1.50 | 10 | 178K | **default (x₀=0)** | 1.000 | **4119** |

QMR строит подпространство Крылова K_m = span{r₀, A·r₀, A²·r₀, ...}. Начальная невязка r₀ определяет базис этого подпространства. При x₀=0 начальная невязка r₀ = b (правая часть), что может давать лучшее покрытие спектра матрицы, чем r₀ = b - A·x₀ при нашем приближении. Более низкая RE₀ **не гарантирует** меньше итераций для QMR.

#### BiCGStab — начальное поле помогает при m ≥ 1.5

| Частица | m | dpl | Диполи | Init | RE₀ | Итерации | Экономия |
|---------|-----|-----|--------|------|-----|----------|----------|
| Куб 3µm | 1.50 | 10 | 178K | **analytical** | 0.458 | **2257** | **-3.7%** |
| Куб 3µm | 1.50 | 10 | 178K | default (x₀=0) | 1.000 | 2345 | — |
| Hex 5×2.5µm | 1.50 | 10 | 135K | **analytical** | 0.544 | **2185** | **-3.6%** |
| Hex 5×2.5µm | 1.50 | 10 | 135K | default (x₀=0) | 1.000 | 2266 | — |
| Hex 10×5µm | 1.50 | 10 | 1.08M | **analytical** | 0.447 | **~7804*** | **~3–4%*** |
| Куб 5µm | 1.3116 | 10 | 831K | analytical | 0.327 | 638 | +16% хуже |
| Куб 5µm | 1.3116 | 10 | 831K | **default (x₀=0)** | 1.000 | **548** | — |

*Hex 10×5µm: завершена только Y-поляризация (7804 итерации); default не запускался.

**Выводы по итерациям:**
1. **QMR** (дефолтный солвер ADDA): наше начальное поле **не ускоряет** сходимость. x₀=0 стабильно лучше.
2. **BiCGStab + m ≥ 1.5**: экономия **~3.5%** итераций. Стабильно воспроизводится на разных размерах и формах.
3. **BiCGStab + лёд (n=1.3116)**: наше поле **ухудшает** сходимость (+16%). Слишком слабый контраст — GO-приближение не даёт преимуществ.
4. С ростом размера частицы общее число итераций растёт (2200 → 7800 для hex), но процент экономии остаётся ~3.5%.
