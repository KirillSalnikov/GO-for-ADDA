...
| `--diffr` | Включить дифракционное сглаживание Кирхгофа для GO-отражений |
...
| GO-отражения | **0.238** | **0.238** |
| `--fp` (Фабри-Перо) | 0.241 | 0.241 |

При нормальном падении GO-отражения дают **лучший** результат: -22% vs norefl. Куб с параллельными гранями — идеальный случай для GO.
...
| GO-отражения | 0.446 | 0.342 |
| `--fp` (1 пара top-bottom) | 0.334 | 0.251 |

Даже для куба с 3 парами антипараллельных граней GO-отражения **сильно вредят** при наклонном падении (+42%). FP тоже ухудшает (+6%), хотя и значительно меньше. Проблема фундаментальная — не в геометрии граней.
...
- `AccumulateReflectedBeams(segments, incidentDir, maxJonesNorm, useDiffraction)` — обрабатывает GO-сегменты с nActs=1 (однократно отражённые пучки). Аддитивное накопление (+=) поверх прямой волны. Per-dipole фаза: обратная проекция на грань отражения → входная грань → OP = FAR_ZONE + incDir·P + n·(d₁+d₂). Фильтр по норме Джонса (|J| > 0.5) отсеивает пучки полного внутреннего отражения. При `useDiffraction=true` (`--diffr`): интеграл Кирхгофа с адаптивной квадратурой (теорема Грина, 8×nSub точек на ребро, координаты в плоскости перпендикулярной пучку, обход CCW). При `useDiffraction=false` (по умолчанию): жёсткие границы (IsInsidePolygon)
